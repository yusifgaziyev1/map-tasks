...
function getMapSet() {
    let map = new Map();
    users.filter((item) => map.set(item.id,item));
    try {
        if (map) {
            console.log(map)
        } else {
...
function getFindRole(id) {;
    users.find((item) => item.id == id ? console.log(`${item.firstName} : ${item.role}`) : null);
}
